...
profile.set_shape("input", min=(1, 1), opt=(8, 1024), max=(32, 4096))
...
PyTorch eager 模式（动态图）：
```python
for op in graph:
    dispatch(op) → 执行
```
→ 每个 op 都有 Python 解释器开销

PyTorch graph 模式（torch.compile）：
```python
graph = capture(model)  # 编译期拓扑排序
execute(graph)          # 一次执行整个图
```
→ 减少 90% Python 开销
...
Stream 0（计算）: [Matmul 1] ──────── [Matmul 2] ────────
Stream 1（搬运）:      [H2D 数据] ──────── [H2D 数据]
                  ↑ 重叠 ↑ 重叠
...
```python
@triton.jit
def attention_kernel(Q, K, V, O, ...):
    # 一个 kernel 完成 QK^T + Softmax + ·V
    # 中间矩阵不出 SRAM
    ...
```
...
model_repo/
├── model_a/
│   ├── config.pbtxt          # 配置（backend/batch_size/instance）
...
```swift
import MLCSwift
let model = MLCModel(modelPath: "Llama-3-8B-Instruct-q4f16_1")
let response = try await model.generate(prompt: "你好")
```
