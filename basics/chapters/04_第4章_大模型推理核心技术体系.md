...
llm = LLM(model="lmsys/longchat-7b-v1.5-32k")  # 自动用 FA
```
...
# 不同 question 共享 system prompt，命中率 80%+
```
...
response = anthropic.messages.create(
    ...
    messages=[{
        ...
        "content": [
            {
                "type": "text",
                "text": "<long_system_prompt>...",  # 长文档/知识库
                "cache_control": {"type": "ephemeral"}  # 标记缓存
            },
            {"type": "text", "text": user_question}
        ]
    }])
# 命中时成本仅 0.1x（10x 节省），TTFT 降低 40-85%
```
...
```bash
vllm serve <model> \
  --swap-space 16 \         # 16GB CPU 内存用于 swap
  --enable-prefix-caching
```
